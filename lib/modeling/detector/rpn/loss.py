...
from lib.structures.boxlist_ops import boxlist_iou, cat_boxlist
...
def make_rpn_loss_evaluator(box_coder):
    matcher = Matcher(
        config.MODEL.INSTANCE2D.RPN.FG_IOU_THRESHOLD,
        config.MODEL.INSTANCE2D.RPN.BG_IOU_THRESHOLD,
        allow_low_quality_matches=True,
    )

    fg_bg_sampler = BalancedPositiveNegativeSampler(
        config.MODEL.INSTANCE2D.RPN.BATCH_SIZE_PER_IMAGE,
        config.MODEL.INSTANCE2D.RPN.POSITIVE_FRACTION
    )
