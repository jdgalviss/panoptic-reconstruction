...
def make_roi_box_loss_evaluator():
    matcher = Matcher(
        config.MODEL.INSTANCE2D.ROI_HEADS.FG_IOU_THRESHOLD,
        config.MODEL.INSTANCE2D.ROI_HEADS.BG_IOU_THRESHOLD,
    ...
    fg_bg_sampler = BalancedPositiveNegativeSampler(
        config.MODEL.INSTANCE2D.ROI_HEADS.BATCH_SIZE_PER_IMAGE, config.MODEL.INSTANCE2D.ROI_HEADS.POSITIVE_FRACTION
    )
