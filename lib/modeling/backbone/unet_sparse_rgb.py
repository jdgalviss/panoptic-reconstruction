...
class UNetSparseRGB(nn.Module):
    ...
    def forward(self, x: torch.Tensor, batch_size, frustum_mask) -> BlockContent:
        # print("\n\nUNetSparseRGB input shape:", x.shape)
...
class UNetBlockOuterSparse(UNetBlock):
    ...
    def forward(self, x: BlockContent, batch_size: int):
        ...
        start_features = end_features
        end_features += config.MODEL.INSTANCE2D.MAX + 1 # skip instance features
        ...
        if self.verbose:
            self.verbose = False
        return BlockContent([output, [proxy_output, proxy_rgb, proxy_surface], dense], processed.encoding)
...
class UNetBlockHybridSparse(UNetBlockOuter):
    ...
    def __init__(self,
        ...
        num_conv_input_features = num_outer_features + 1 # +1 for the occupancy
    ...
    def forward(self, x: BlockContent, batch_size: int) -> BlockContent:
        ...
        proxy_output = [proxy_output, rgb_prediction, surface_prediction]
