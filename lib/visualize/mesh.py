...
def write_distance_field(distance_field: Union[np.array, torch.Tensor], labels: Optional[Union[np.array, torch.Tensor]],
                         ...
                         color_palette=None, transform=None, is_color=False) -> None:
    ...
    if is_color:
        vertices, colors, triangles = get_mesh_with_rgb(distance_field, labels, iso_value, truncation)
    else:
...
def get_mesh_with_rgb(distance_field: np.array, colors: np.array, iso_value: float = 1.0,
                         truncation: float = 3.0) -> Tuple[np.array, np.array, np.array]:
    vertices, triangles = mc.marching_cubes_color(distance_field, colors, iso_value, truncation)
    colors = vertices[..., 3:]
